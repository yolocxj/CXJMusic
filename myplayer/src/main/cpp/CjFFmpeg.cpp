...
CjFFmpeg::CjFFmpeg(CjPlaystatus *playstatus, CjCallJava *callJava, const char *url) {
...
{
    CjFFmpeg *cjFFmpeg = (CjFFmpeg *) data;
    cjFFmpeg->decodeFFmpegThread();
    pthread_exit(&cjFFmpeg->decodeThread);
}

void CjFFmpeg::parpared() {
...
void CjFFmpeg::decodeFFmpegThread() {
    ...
    for(int i = 0; i < pFormatCtx->nb_streams; i++)
    {
        if(pFormatCtx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO)//得到音频流
        {
            if(audio == NULL)
            {
                audio = new CjAudio(playstatus, pFormatCtx->streams[i]->codecpar->sample_rate, callJava);
                audio->streamIndex = i;
...
void CjFFmpeg::seek(int64_t secds) {
...
void CjFFmpeg::setVolume(int percent) {
    if(audio != NULL)
    {
...
void CjFFmpeg::setMute(int mute) {
    if(audio != NULL)
    {
        audio->setMute(mute);
    }
...
void CjFFmpeg::setPitch(float pitch) {
...
void CjFFmpeg::setSpeed(float speed) {
...
void CjFFmpeg::startStopRecord(bool start) {
    if(audio != NULL)
    {
...
bool CjFFmpeg::cutAudioPlay(int start_time, int end_time, bool showPcm) {
