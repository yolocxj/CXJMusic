...
CjAudio::CjAudio(CjPlaystatus *playstatus, int sample_rate, CjCallJava *callJava) {
...
{
    CjAudio *cjAudio = (CjAudio *) data;

    cjAudio->initOpenSLES();

    pthread_exit(&cjAudio->thread_play);
}

void *pcmCallBack(void *data)
{
    CjAudio *audio = static_cast<CjAudio *>(data);
    while(audio->playstatus != NULL && !audio->playstatus->exit)
    {
        CjPcmBean *pcmBean = NULL;
...
{
    CjAudio *cjAudio = (CjAudio *) context;
    if(cjAudio != NULL)
    {
        int buffersize = cjAudio->getSoundTouchData();
        if(buffersize > 0)
        {
            cjAudio->clock += buffersize / ((double)(cjAudio->sample_rate * 2 * 2));
            if(cjAudio->clock - cjAudio->last_tiem >= 0.1)
            {
                cjAudio->last_tiem = cjAudio->clock;
                //回调应用层
                cjAudio->callJava->onCallTimeInfo(CHILD_THREAD, cjAudio->clock, cjAudio->duration);
            }

            cjAudio->bufferQueue->putBuffer(cjAudio->sampleBuffer, buffersize * 4);
            cjAudio->callJava->onCallValumeDB(CHILD_THREAD,
            cjAudio->getPCMDB(reinterpret_cast<char *>(cjAudio->sampleBuffer), buffersize * 4));
            (* cjAudio-> pcmBufferQueue)->Enqueue( cjAudio->pcmBufferQueue, (char *) cjAudio->sampleBuffer, buffersize * 2 * 2);

            if(cjAudio->isCut)
            {
                if(cjAudio->clock > cjAudio->end_time)
                {
                    LOGE("裁剪退出...");
                    cjAudio->playstatus->exit = true;
                }
            }
...
int CjAudio::getCurrentSampleRateForOpensles(int sample_rate) {
    int rate = 0;
    switch (sample_rate)
    {
        case 8000:
...
void CjAudio::setVolume(int percent) {
...
void CjAudio::setPitch(float pitch) {
    this->pitch = pitch;
    if(soundTouch != NULL)
    {
        soundTouch->setPitch(pitch);
...
void CjAudio::setSpeed(float speed) {
    this->speed = speed;
    if(soundTouch != NULL)
    {
        soundTouch->setTempo(speed);
...
int CjAudio::getPCMDB(char *pcmcata, size_t pcmsize) {
    int db = 0;
...
void CjAudio::startStopRecord(bool start) {
